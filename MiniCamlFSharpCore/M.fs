module mini_caml_fsharp_core.M
...
type 't M =
    ...
    member this.Map m = this.env |> Map.map m |> M.OfMap

    member this.AddList xys =
        List.fold (fun env (k, v) -> env |> Map.add k v) this.env xys |> M.OfMap

    member this.AddList2 xs ys =
        List.fold2 (fun env k v -> env |> Map.add k v) this.env xs ys |> M.OfMap
