module mini_caml_fsharp_core.S
...
type S =
    ...
    member this.Exclude s =
        Set.difference this.set s.set |> S.OfSet

    member this.Intersect s =
        this.set |> Set.intersect s.set |> S.OfSet

    member this.IsSubsetOf s = Set.isSubset this.set s.set
    member this.IsSupersetOf s = Set.isSuperset this.set s.set
    member this.Remove x = this.set |> Set.remove x |> S.OfSet

    member this.AddList xs =
        List.fold (fun set x -> set |> Set.add x) this.set xs |> S.OfSet
