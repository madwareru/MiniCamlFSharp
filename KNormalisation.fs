...
module KNormalisation =
    ...
    and binary ctr t env l r =
        (g env l) |> insert_let (fun l' ->
            (g env r) |> insert_let (fun y' ->
                ctr (l', y'), t))
    and g (env : Type.t M ) =
        function
        ...
        | Syntax.EqNode _ | Syntax.LENode _ as cmp ->
            Syntax.IfNode(
                cmp,
                Syntax.BoolNode(true),
                Syntax.BoolNode(false)
        ...
        | Syntax.IfNode(Syntax.NotNode(cond_e), then_e, else_e) ->
            Syntax.IfNode(cond_e, else_e, then_e) |> g env
        | Syntax.IfNode(Syntax.EqNode(l, r), then_e, else_e) ->
            (g env l) |> insert_let (fun l' ->
                (g env r) |> insert_let (fun r' ->
        ...
        | Syntax.IfNode(cond_e, then_e, else_e) -> 
            Syntax.IfNode( 
                Syntax.EqNode(cond_e, Syntax.BoolNode false),
                else_e,
                then_e
            ) |> g env
        ...
        | Syntax.ApplyNode(callee_e, exs) ->
            ...
            | _, Type.FunType(_, t) as g_e ->
                g_e |> insert_let (fun f ->
                    let rec bind ids =
                        function
                        | [] -> KNorm.Apply(f, ids |> List.rev), t
                        | e :: rest ->
                            g env e |> insert_let (fun id -> bind (id :: ids) rest)
                    bind [] exs)
        ...
        | Syntax.ArrayNode(v_e, count_e) ->
            g env count_e |> insert_let ( fun count_e' ->
                let _, v_t as g_v_e = g env v_e
                g_v_e |> insert_let ( fun v_e' ->
                    let l =
                        match v_t with
        ...
        | Syntax.GetNode(arr_e, ix_e) ->
            ...
            | _, Type.ArrayType(t) as g_arr_e ->
                g_arr_e |> insert_let (fun arr_e' ->
                    g env ix_e |> insert_let (fun ix_e' -> KNorm.Get(arr_e', ix_e'), t))
            | _ -> failwith "failed to k-normalize Get node (first operand is not an array)"
        | Syntax.PutNode(arr_e, ix_e, v_e) ->
            g env arr_e |> insert_let ( fun arr_e' ->
                g env ix_e |> insert_let ( fun ix_e' ->
                    g env v_e |> insert_let ( fun v_e' ->
                        KNorm.Put(arr_e', ix_e', v_e'), Type.UnitType)))
            
        | Syntax.LetNode((name, t), binding_e, cont_e) ->
            let binding_e', _ = g env binding_e
            let env' = env.Add name t
            let cont_e', cont_t = g env' cont_e
            KNorm.Let((name, t), binding_e', cont_e'), cont_t
